...
const getDomain = (games: View<"formVisualization">["games"]) => {
	const values = games
...
const Chart = ({
	...
	games: View<"formVisualization">["games"];
	metricLabel: string;
}) => {
	const margin = {
		bottom: 35,
		...
		<div className="position-relative">
			...
			{tooltipOpen && tooltipData ? (
				<TooltipWithBounds left={tooltipLeft} top={tooltipTop}>
					...
					{tooltipData.formIsEstimate ? " (estimated form)" : ""}
				</TooltipWithBounds>
			) : null}
		</div>
	);
...
const FormVisualization = ({
	...
	season,
	windowSize,
}: View<"formVisualization">) => {
	useTitleBar({
		title: "Form Visualization",
		dropdownView: "form_visualization",
	});
	...
	const expectedSuffix = `/form_visualization/${season}/${pid ?? 0}/${metric}/${windowSize}/${minMinutes}`;
	if (
		location.pathname.includes("/form_visualization") &&
	...
	const rows: DataTableRow[] = games.map((game) => ({
		...
		data: [
			...
			game.formIsEstimate && metric === "form" ? "Estimated" : "Exact",
		],
	}));
				...
				<>
					<p className="mb-3">
						...
						{metric === "form" ? (
							<>
								{" "}
								Exact tracked games: {numExactGames}. Estimated form games:{" "}
								{numEstimatedGames}.
							</>
						) : null}
					</p>
					...
					<p className="text-body-secondary mt-3">
						The chart plots eligible games only. For a rolling window above 1,
						the line shows a recent {windowSize}-game value, while the table
						also shows each game's raw value. Basketball advanced stats are
						computed from the aggregated box scores inside each window.
					</p>
