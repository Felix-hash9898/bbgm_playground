...
export const updateNba2027AfterLottery = async (tidsTop5: number[]) => {
	...
	for (const t of await idb.cache.teams.getAll()) {
		...
		if (t.tid === firstTid) restricted1 = true;
		if (tidsTop5.includes(t.tid)) restricted5 = true;
		if (restricted1 || restricted5) {
			const prev =
				t.draftLottery?.type === "nba2027"
					...
					: undefined;
			t.draftLottery = { type: "nba2027" };
			if (restricted1) t.draftLottery.restricted1 = true;
			if (restricted5)
...
export const initializeNba2027 = async () => {
	...
	const applyRows = (
		...
		for (const row of rows) {
			...
			if (row.pick !== undefined && row.pick <= RESTRICTED_5_PICK) {
				...
				else if (!older) restricted5ByTid[row.originalTid] = 1;
			}
		}
	};
	...
	applyRows(results?.result ?? (await loadRows(lastSeason)), false);
	applyRows(await loadRows(lastSeason - 1), true);

	for (const t of teams) {
		if (t.disabled || t.draftLottery?.type === "nba2027") continue;
		const restricted1 = restricted1ByTid[t.tid];
		const restricted5 = restricted5ByTid[t.tid];
		if (restricted1 || restricted5) {
			t.draftLottery = { type: "nba2027" };
			if (restricted1) t.draftLottery.restricted1 = true;
			if (restricted5) t.draftLottery.restricted5 = restricted5;
		} else delete t.draftLottery;
		await idb.cache.teams.put(t);
	}
};
